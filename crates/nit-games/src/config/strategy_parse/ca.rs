...
const CA_DEFAULT_K: usize = 2;
const CA_DEFAULT_T: u32 = 10;
const CA_MAX_TABLE_ENTRIES: u128 = 1_000_000;
...
pub(in crate::config) fn normalize_ca_kind(
    ...
    let k_raw = raw.k.unwrap_or(CA_DEFAULT_K);
    ...
    let two_r = neighborhood_radius_to_diameter(r_raw).unwrap_or_else(|| {
        errors.push(format!(
            "strategy '{id}': ca.r must satisfy r >= 0 and IntegerQ[2r]"
        ));
        0
    });
    let t = raw.t.or(raw.steps).unwrap_or(CA_DEFAULT_T);
    if t == 0 {
        errors.push(format!("strategy '{id}': ca.t must be > 0"));
    }

    validate_ca_table_size(id, k, two_r, errors);
...
fn validate_ca_table_size(id: &str, k: u8, two_r: u32, errors: &mut Vec<String>) {
    let neighborhood = two_r.saturating_add(1);
    match checked_pow_u128(k as u128, neighborhood) {
        Some(table_len) if table_len > CA_MAX_TABLE_ENTRIES => {
            errors.push(format!(
...
/// Converts CA neighbourhood radius `r` to diameter `2r` as a non-negative
/// integer. Returns `None` if `r` is non-finite, negative, or `2r` would not
/// be an integer (the configuration grammar requires `IntegerQ[2r]`).
