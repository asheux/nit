...
fn ca_rule_zero_and_255_match_notebook_behavior() {
    ...
    let empty = History::new(0);
    assert_eq!(ca_zero.next_action(&empty, true), Action::Cooperate);
    assert_eq!(ca_255.next_action(&empty, true), Action::Cooperate);
...
fn ca_nontrivial_rule_matches_hand_computation() {
    // r=1 ⇒ neighborhood width 3. Rule n=85 maps each neighborhood to its
    // rightmost bit. Hand-trace: history bits = [1,0,1,1]; windows
    // [1,0,1]→1, [0,1,1]→1; final row [1,1] last cell 1 ⇒ Defect.
    let mut history = History::new(0);
    record_round(&mut history, Action::Defect, Action::Cooperate);
    record_round(&mut history, Action::Defect, Action::Defect);
