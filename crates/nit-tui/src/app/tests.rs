...
/// Bug 4: the multipane runtime now uses the same per-event drain
/// helper as single-pane. A planner `TurnCompleted` carrying a parsed
/// plan must drive `swarm.handle_event_outcome` so the run progresses
/// past Planning and emits stage-2 dispatches. Without the helper,
/// the swarm would freeze after the planner with breathers stuck on
/// "Waiting…".
#[test]
