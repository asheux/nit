...
pub(crate) fn role_contract_lines(role: &str) -> &'static [&'static str] {
    match role {
        ...
        "judge" => &[
            ...
            "FILES ARRAY = SCOPE HINT: your `swarm_artifacts.files` is rendered to the integrator as a starting hint about which files are likely to need editing. Your VERDICT PROSE is what binds the integrator — the prose is the contract; the files array is advisory. The runtime no longer auto-re-dispatches integrators who skip files in your list (an Info diag surfaces the gap for operator visibility, but the structural-compliance loop is OFF by default; operators who want it restored set `NIT_STRICT_CHECKLIST=1`). The integrator's role contract also explicitly authorises edits OUTSIDE the listed files when completing the work the verdict endorsed requires touching adjacent code (wiring a keybinding, updating callers of a changed signature, etc.). So: pick the files you think your verdict requires the integrator to touch, but write your verdict as if the prose were the only contract — because it is. If you describe a split in prose but only list the source file in `files`, the integrator follows the prose and creates the submodules. If you list 8 split paths but your prose rejects the split, the integrator follows the prose and leaves them alone. Stub-file creation and incomplete-split detection (newly-created declared files under 20 lines, or huge sources that didn't shrink while sibling submodules appeared) are still enforced — those are work-not-done signals regardless of the file list.",
        ],
        "integrate" => &[
            ...
            "If a FILE CHECKLIST is provided above, it's an INFORMATIONAL HINT about which files the proposers / judge identified as likely targets. The JUDGE'S VERDICT PROSE in the dependency outputs is the authoritative source for what to build. Process the checklist as a starting point: open each file, read it, and edit it WHEN the verdict requires it. Files in the checklist you decide are audit-only (no edit needed) go in your final message under an `audit-only` heading with a one-line reason — they don't count against task completion. Files NOT in the checklist that you DO need to edit (callers of a changed signature, keybinding registration sites, lib.rs re-exports) ARE allowed and expected; name them under an `out-of-scope edits` heading. The runtime's missing-file re-dispatch loop is off by default; the operator opt-in is `NIT_STRICT_CHECKLIST=1`.",
            ...
            "COMPLETION-MANDATED OUT-OF-SCOPE EDITS — ALLOWED: when the declared task cannot be SHIPPED end-to-end without modifying a file outside your FILE CHECKLIST, you ARE PERMITTED — and EXPECTED — to make that out-of-scope edit. Common shapes the runtime sees this fail on, and how to handle them:\n\
            \n\
            - **Wiring a new keybinding**: needs Action variant in `crates/nit-core/src/actions.rs`, an arm in `crates/nit-core/src/state/action_apply.rs`, the binding in `crates/nit-tui/src/app/key_dispatch.rs`, AND a render-side hookup (e.g. `crates/nit-tui/src/app/draw.rs` or the relevant widget). If your checklist only lists the widget, modify the other three anyway — a binding that doesn't dispatch is a non-feature.\n\
            - **Function signature change**: needs every caller updated. Modify every caller even when they live in files outside your scope.\n\
            - **New struct field**: needs every constructor + every destructuring match updated. Touch them all.\n\
            - **New public re-export**: needs the matching `pub use` in `crates/<crate>/src/lib.rs`. Add it.\n\
            \n\
            What this permission does NOT authorize:\n\
            - Reintroducing a refactor the judge explicitly REJECTED in its verdict prose (e.g. \"AUDIT-ONLY files: do not create\"). The scope unlock applies to completion of YOUR plan, not to ones the judge already declined.\n\
            - Adding features the operator didn't ask for, even if they'd be nice.\n\
            - Refactoring unrelated code that happens to live in a file you're already touching.\n\
            \n\
            For every out-of-scope edit, name the file in your final message under an `out-of-scope edits` heading with a one-line reason (e.g. \"`crates/nit-tui/src/app/key_dispatch.rs` — registered the Ctrl+Shift+T binding so the popup widget is reachable\"). The reviewer audits these for the rejected-refactor pattern; legitimate completion edits are accepted, scope-creep edits are flagged.",
...
fn append_task_integrate_checklist(out: &mut String, scope_files: &[String]) {
    out.push_str("\n## FILE CHECKLIST (informational hint)\n");
    out.push_str("This list is sourced from the propose/judge `swarm_artifacts.files` arrays in your dependency outputs — it's the set of files the proposers and judge identified as LIKELY targets for your work. Treat it as a starting hint, not a contract.\n");
    out.push('\n');
    out.push_str("Authoritative source: the JUDGE'S VERDICT in the Dependency outputs section below. Follow the verdict's specific choices end-to-end. The checklist exists so you don't have to re-derive the scope from scratch; the verdict exists to tell you what to actually build.\n");
    out.push('\n');
    out.push_str("How to use it:\n");
    out.push_str("- Process the list as a starting point: open each file, read it, decide whether the work declared in the verdict actually requires editing it.\n");
    out.push_str("- A file listed here that you decide doesn't need editing is acceptable — say so in your final message under an `audit-only` heading with a one-line reason (e.g. \"core/diagnostics.py — judge endorsed in-place refactor; this file's surface area was unaffected\").\n");
    out.push_str("- A file NOT listed here that you DO need to edit is also acceptable — your role contract's COMPLETION-MANDATED OUT-OF-SCOPE EDITS clause authorises edits anywhere the work requires. Name them under an `out-of-scope edits` heading with a one-line reason.\n");
    out.push_str("- The runtime's structural compliance check is now ADVISORY for missing files — an Info diag in soft mode (the default), only a re-dispatch under `NIT_STRICT_CHECKLIST=1`. Stub files (newly-created files under 20 lines) and incomplete splits (a declared huge source that didn't shrink while sibling submodules appeared) DO still trigger re-dispatch — those are genuine work-not-done signals regardless of the checklist contract.\n");
    out.push('\n');
    out.push_str("Other persistent rules:\n");
    out.push_str("- Do NOT add inline test modules (`#[cfg(test)] mod tests { ... }`) inside source files. Tests live in a dedicated tests directory or test file.\n");
    out.push_str("- COMMENTS: trim doc comments that restate the type/function name, echo visible type signatures, or describe obvious behavior (e.g. \"/// Returns the value\" on `fn value()`). Keep comments that explain WHY, document non-obvious constraints, safety invariants, or algorithmic choices.\n");
    out.push_str("- \"Refactor module\" still means \"do the refactor the judge endorsed across the whole module\" — partial coverage is a task failure, just one detected by the reviewer / compile gate / genome retry rather than the FILE CHECKLIST.\n");
    out.push('\n');
    for (i, path) in scope_files.iter().enumerate() {
        out.push_str(&format!("{}. {path}\n", i + 1));
    }
    out.push_str("\nAfter finishing, list every file you actually changed and what you changed in each. Out-of-scope edits and audit-only decisions go in their own sub-sections so the reviewer can audit.\n");
}
