...
pub fn render_body(frame: &mut Frame<'_>, inner: Rect, state: &mut AppState, theme: &Theme) {
    ...
    state.substrate_assumptions_last_max_scroll = max_scroll;
    let scroll = state.substrate_assumptions_scroll.min(max_scroll) as u16;
    state.substrate_assumptions_scroll = scroll as usize;
    frame.render_widget(
        Paragraph::new(lines)
