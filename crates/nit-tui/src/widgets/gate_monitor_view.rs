...
pub fn render(frame: &mut Frame, area: ratatui::layout::Rect, state: &AppState, theme: &Theme) {
    ...
    let syntax_debug = state.syntax_debug.as_ref();
    ...
    for (idx, entry) in state.visualizer.leaderboard.iter().take(3).enumerate() {
        rows.push(row(
            format!("Rule {}", idx + 1),
            format!("{} ({:.1})", entry.rule, entry.score),
            label_style,
            leaderboard_style(idx, theme, value_style),
        ));
    }

    let lines = format_rows(rows, 14, inner.width as usize);
...
fn render_games(frame: &mut Frame, area: ratatui::layout::Rect, state: &AppState, theme: &Theme) {
    ...
    let lines = format_rows(rows, 12, inner.width as usize);
    let lines = apply_ui_selection(
        lines,
        state.ui_selection.as_ref(),
        UiSelectionPane::GateMonitor,
...
pub fn build_lines(state: &AppState, theme: &Theme, width: usize) -> Vec<Line<'static>> {
    if state.app_kind == AppKind::Games {
        ...
        build_lines_gol(state, theme, width)
    }
}

fn build_lines_gol(state: &AppState, theme: &Theme, width: usize) -> Vec<Line<'static>> {
    let (ln, col) = state.line_col();
    let petri_mode = match state.visualizer.mode {
        nit_core::VisualizerMode::SimOnly => "SIM",
        nit_core::VisualizerMode::Search => "SEARCH",
