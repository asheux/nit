//! Integration tests for the root `nit_utils` module.

use nit_utils::hashing::stable_hash_bytes;
use nit_utils::{content_tag, ensure_dir, ContentTag, Fingerprint};

#[test]
fn fingerprint_on_byte_slice_matches_direct_hash() {
    let via_function = stable_hash_bytes(b"hello");
    let via_trait: u64 = b"hello".as_slice().fingerprint();
    assert_eq!(via_function, via_trait);
}

#[test]
fn fingerprint_on_str_matches_byte_hash() {
    let str_digest = "world".fingerprint();
    let byte_digest = stable_hash_bytes(b"world");
    assert_eq!(str_digest, byte_digest);
}

#[test]
fn fingerprint_string_delegates_to_str() {
    let owned = String::from("test");
    let borrowed = "test".fingerprint();
    assert_eq!(owned.fingerprint(), borrowed);
}

#[test]
fn fingerprint_vec_delegates_to_slice() {
    let owned = vec![1u8, 2, 3];
    let borrowed = [1u8, 2, 3].as_slice().fingerprint();
    assert_eq!(owned.fingerprint(), borrowed);
}

#[test]
fn content_tag_format_and_determinism() {
    let generated = content_tag("v2", b"payload");
    assert!(generated.starts_with("v2-"));
    assert_eq!(generated.len(), "v2-".len() + 8);
    assert_eq!(generated, content_tag("v2", b"payload"));
}
...
fn ensure_dir_creates_and_returns_target() {
    let scratch = std::env::temp_dir().join(format!("nit_lib_{}", std::process::id()));
    let returned = ensure_dir(&scratch).expect("mkdir failed");
