...
// Per-family ceilings on generated machine counts. Metal lifts the cap by
// an order of magnitude because GPU dispatch absorbs the per-machine fixed
// cost; CPU pipelines get the smaller bound.
pub(super) const MAX_FAMILY_RUN_MACHINES_CPU: usize = 10_000;
const MAX_FAMILY_RUN_MACHINES_METAL: usize = 100_000;
// Defaults from Code-02 / Results-06 notebooks. Keeping them in sync lets
// existing analyses replay against new family runs.
...
// CPU budget guards: matches × rounds_per_match grow multiplicatively, so
// we bound both terms before normalizing the config.
...
/// Build a family-run override from `config_text`, discarding timings.
/// Callers that want timing diagnostics use the `_with_timings` form below.
...
/// Build a family-run override from a pre-parsed `FamilyRunBaseConfig`.
/// Skips the TOML re-parse done by `build_family_run_override_for_request`
/// — callers that already hold a parsed config use this entry point.
