...
pub fn build_family_run_override_for_request(
    ...
    let (strategies, label) = match request.family.as_str() {
        ...
        "tm" => {
            ...
            let (blank, default_max_steps) = default_tm_family_params(&config);
    ...
    config = nit_games::select_halting_turing_machine_strategies(config);
...
fn default_tm_family_params(config: &nit_games::NormalizedConfig) -> (u8, u32) {
    ...
    let blank = config
        ...
        .find_map(|spec| match &spec.kind {
            nit_games::config::StrategySpecKind::OneSidedTm { blank, .. } => Some(*blank),
            _ => None,
        })
        .unwrap_or(0);
