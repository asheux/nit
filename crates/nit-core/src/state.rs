...
const MAX_FAMILY_RUN_MACHINES: usize = 10_000;
const DEFAULT_FAMILY_CA_T: u32 = 10;
const DEFAULT_FAMILY_TM_MAX_STEPS: u32 = 256;
const FAMILY_RUN_CONFIG_PREFIX: &str = "# generated by :games run <family> {...}";
const MAX_FAMILY_TOTAL_MATCHES: u128 = 500_000;
...
fn estimate_total_matches(
    ...
    base.checked_mul(reps)
        .ok_or_else(|| "Family run match-count overflow".to_string())
}

fn parse_braced_parts<'a>(input: &'a str, usage: &str) -> Result<Vec<&'a str>, String> {
...
fn parse_tm_family_tuple(input: &str) -> Result<(u16, u8), String> {
    let usage = "Usage: :games run tm {states, symbols}";
    let parts = parse_braced_parts(input, usage)?;
    if parts.len() != 2 {
    ...
    Ok((states_raw as u16, symbols_raw as u8))
}
...
fn checked_machine_count(count: u128, family: &str) -> Result<usize, String> {
    if count == 0 {
        return Err(format!("{family} family has zero machines"));
    }
    if count > MAX_FAMILY_RUN_MACHINES as u128 {
        return Err(format!(
            "{family} family run would generate {count} machines; limit is {MAX_FAMILY_RUN_MACHINES}"
        ));
    }
    ...
    config
        ...
        .find_map(|spec| match &spec.kind {
            nit_games::config::StrategySpecKind::OneSidedTm {
                ...
                max_steps_per_round,
                ..
            } => Some((*blank, *max_steps_per_round)),
            _ => None,
        })
        .unwrap_or((0, DEFAULT_FAMILY_TM_MAX_STEPS))
}
...
fn generate_tm_family_strategies(
    ...
    let count = max_index
        .checked_add(1)
        .ok_or_else(|| "TM family index space overflow".to_string())?;
    let machine_count = checked_machine_count(count, "TM")?;
    let mut specs = Vec::with_capacity(machine_count);
...
fn generate_ca_family_strategies(
    ...
    let machine_count = checked_machine_count(rule_count, "CA")?;
    let mut specs = Vec::with_capacity(machine_count);
    for index in 0..machine_count {
        let n = index as u64;
        specs.push(nit_games::StrategySpec {
