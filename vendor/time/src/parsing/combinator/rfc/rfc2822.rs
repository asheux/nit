...
// RFC 2822 comments can be nested. Without a bound, deeply nested input can cause stack exhaustion
// due to recursive parsing.
const MAX_COMMENT_NESTING: u8 = 32;
...
fn comment(input: &[u8]) -> Option<ParsedItem<'_, ()>> {
    comment_with_depth(input, 0)
}

#[inline]
fn comment_with_depth(mut input: &[u8], depth: u8) -> Option<ParsedItem<'_, ()>> {
    if depth >= MAX_COMMENT_NESTING {
        return None;
    }
    input = ascii_char::<b'('>(input)?.into_inner();
    input = zero_or_more(fws)(input).into_inner();
    while let Some(rest) = ccontent(input, depth) {
...
fn ccontent(input: &[u8], depth: u8) -> Option<ParsedItem<'_, ()>> {
    ctext(input)
        ...
        .or_else(|| comment_with_depth(input, depth.saturating_add(1)))
}
